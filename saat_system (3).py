...
CONFIG = {
    ...
    # ==========================================================================
    "servo": {
        "home_angle": 80.0,      # resting/idle position, servo returns here after every action  <-- CALIBRATE
        "reject_angle": 0.0,     # eject the pear off the belt                                     <-- CALIBRATE
        "accept_angle": 180.0,   # pass-through / accept gesture                                   <-- CALIBRATE
        "return_delay_s": 0.4,   # how long the servo holds at accept/reject before returning home
        ...
        # --- smooth motion (fixes jerky/"discrete" instant-jump movement) ---
        "move_step_deg": 4.0,      # degrees per interpolation step - smaller = smoother, slower  <-- TUNE
        "move_step_delay_s": 0.012,  # pause between steps, seconds - larger = slower, smoother    <-- TUNE
    },
...
class ServoController:
    def __init__(self, cfg):
        ...
        self._lock = threading.Lock()
        self._current_angle = {z: cfg["home_angle"] for z in MOTOR_CHANNEL}
        ...
        # Move every gate to its resting/home position at startup, smoothly,
        # so nothing snaps on the very first real command.
        for zone in MOTOR_CHANNEL:
            self._move_smooth(zone, cfg["home_angle"], initial=True)
        print(f"[ServoController] all zones parked at home_angle={cfg['home_angle']} deg.")

    def _move_smooth(self, zone: str, target_angle: float, initial: bool = False):
        """Interpolates from the servo's last known angle to target_angle in
        small steps instead of snapping directly there. This is what fixes
        the jerky/'discrete' instant-jump motion - the physical servo now
        sweeps smoothly, which is both gentler on the MG995 gearbox and much
        less violent on the pear/gate hardware. Tune CONFIG["servo"]
        ["move_step_deg"] / ["move_step_delay_s"] for faster vs. smoother.
        Only the actual angle write is locked (briefly) - the sweep itself
        does NOT hold the lock, so all 6 zones can sweep concurrently
        without stalling each other's action cycle."""
        channel = MOTOR_CHANNEL[zone]
        step = self.cfg["move_step_deg"]
        delay = self.cfg["move_step_delay_s"]
        current = self._current_angle[zone] if not initial else target_angle

        if not self.hardware_active:
            print(f"[servo:sim] {zone} (ch {channel}) -> {target_angle:.0f} deg"
                  + ("" if initial else f" (from {current:.0f} deg, smooth)"))
            self._current_angle[zone] = target_angle
            return

        direction = 1.0 if target_angle >= current else -1.0
        angle = current
        while abs(target_angle - angle) > step:
            ...
            time.sleep(delay)
        with self._lock:
            self.kit.servo[channel].angle = max(0.0, min(180.0, target_angle))
        self._current_angle[zone] = target_angle

    def set_angle(self, zone: str, angle: float):
        """Direct/instant set - kept for callers that explicitly want no
        interpolation. Normal operation uses _move_smooth() via dispatch()."""
        channel = MOTOR_CHANNEL[zone]
        with self._lock:
            if self.hardware_active:
                self.kit.servo[channel].angle = angle
            else:
                print(f"[servo:sim] {zone} (ch {channel}) -> {angle:.0f} deg (direct)")
            self._current_angle[zone] = angle

    def dispatch(self, zone: str, accepted: bool):
        """Section 12: only the ACTION_NODE for this zone may move hardware.
        Three fixed positions per CONFIG["servo"]: home (idle/resting),
        accept, reject. On every decision the gate sweeps smoothly out to
        the accept/reject angle, holds for return_delay_s, then sweeps
        smoothly back to home."""
        target = self.cfg["accept_angle"] if accepted else self.cfg["reject_angle"]
        self._move_smooth(zone, target)
        time.sleep(self.cfg["return_delay_s"])
        self._move_smooth(zone, self.cfg["home_angle"])
